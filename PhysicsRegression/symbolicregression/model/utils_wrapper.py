...
class BFGSRefinement():
    ...
    def go(
        ...
        if safely_refine:
            func = env.simplifier.safely_tree_to_torch_module(tree, dtype=torch.float64)
        else:
        ...
        self.X = torch.from_numpy(self.X).to(torch.float64).requires_grad_(False)
        self.y = torch.from_numpy(self.y).to(torch.float64).requires_grad_(False)
        ...
        def objective_torch(coeffs):
            ...
            #mse_norm = ((self.y - y_tilde) / torch.abs(self.y)).pow(2).mean().div(2)
