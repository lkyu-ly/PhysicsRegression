...
class TransformerModel(nn.Module):
    ...
    def __init__(
        ...
        if params.use_dimension_mask:
            ...
            self.variables_maxi_num = max([
                i if self.id2word[self.variables_start_index + i - 1] == f"x_{i-1}" else 0
                for i in range(1, 100)
            ])

            #dimensionless unary
            self.dimensionless_op_index = torch.tensor([
                self.word2id[op]
                for op in ["arccos", "arcsin", "arctan", "cos", "cosh", "exp", "log", "pi", "sin", "sinh", "tan"]
            ])
            self.dimensionless_dimension_index = torch.tensor([
                self.word2id[dim]
                for dim in ["M0", "S0", "K0", "T0", "V0"]
            ])

            #self.dimensionless_op_mask = torch.tensor([
            #    True if i in self.dimensionless_op_index else False
            #    for i in range(self.n_words)
            #])
            
            #mask for dimension
            self.dimension_start_index = min(
                [self.word2id.get(f"M-{i}", 1e10) for i in range(101)]
            )
            self.dimension_end_index = max(
                [self.word2id.get(f"V{i}", -1e10) for i in range(101)]
            )
            ...
            self.dimension_mask = torch.tensor([
                [
                    True
                    if dimension0[i] - self.dimension_num[i] <= j <= dimension0[i] + self.dimension_num[i]
                    else False
        ...
        if self.is_decoder and params.decode_physical_units == "double-seq":
            self.units_enc = nn.Linear(self.dim*5, self.dim)
            self.units_dec = nn.Linear(self.dim, self.dim*5)
    ...
    def fwd(
        ...
        if self.is_decoder and units is not None:
            ...
            units_tensor = units_tensor.reshape((units_tensor.shape[0], units_tensor.shape[1], -1))
    ...
    def predict(self, tensor, pred_mask, y, y_units=None, get_scores=False):
        ...
        next_word = torch.topk(scores, 1)[1].squeeze(1)
        if y_units is not None:
            ...
            loss_units = F.cross_entropy(scores_units.float(), y_units, reduction="mean")
            next_unit = torch.topk(scores_units, 1)[1].squeeze(1)
            loss = loss + loss_units * 0.2
        return scores, loss

    def generate(
        self, src_enc, src_len, decode_physical_units,
        max_len=200, top_p=1.0, sample_temperature=None, units=None, 
    ):
        """
        ...
        if decode_physical_units is None or decode_physical_units == "single-seq":
            ...
            #perplexity
            word_perplexity = torch.zeros((bs)).to(src_enc.device)
            ...
            while cur_len < max_len:
                ...
                #perplexity
                next_words_prob = F.softmax(scores.float(), dim=1)              #(bs, n_words)
                next_words_perplexity = next_words_prob[
                    tuple([torch.arange(bs), next_words])
                ]                                                               #(bs)
                assert next_words_perplexity.size() == (bs, )

                word_perplexity.add_(
                    torch.log(next_words_perplexity.detach()) * unfinished_sents
                )
            ...
            if cur_len == max_len:
                # maybe not supported in new version of PyTorch
            ...
            rows, cols = torch.nonzero(generated[1:] == self.eos_index, as_tuple=True)
            word_perplexity = torch.exp(word_perplexity / rows)
            ...
            # generated sentences
            generated1 = src_len.new(max_len, bs)  # upcoming output
            generated1.fill_(self.pad_index)  # fill upcoming ouput with <PAD>
            generated1[0].fill_(self.eos_index)  # we use <EOS> for <BOS> everywhere

            generated2 = src_len.new(max_len, bs, 5)
            generated2.fill_(self.pad_index)
            generated2[0].fill_(self.eos_index)  # we use <EOS> for <BOS> everywhere
            ...
            #perplexity
            word_perplexity = torch.zeros((bs)).to(src_enc.device)
            dimension_perplexity = torch.zeros((bs)).to(src_enc.device)
            ...
            while cur_len < max_len:
                ...
                if self.use_dimension_mask:
                    dimension_mask = torch.ones((bs, self.n_words))
                    required_dimension = generated2[cur_len-1].clone().to("cpu")  # (bs, 5)
                    #print(torch.sum(dimension_mask))

                    #variables
                    variable_allowed = [
                        torch.all(units[i] == required_dimension[i], dim=1)
                        for i in range(bs)
                    ]
                    for i in range(bs):
                        #print(dimension_mask[i][self.variables_start_index: self.variables_start_index+self.variables_maxi_num])
                        dimension_mask[i][self.variables_start_index: self.variables_start_index+len(variable_allowed[i])][~variable_allowed[i]] = 0
                        dimension_mask[i][self.variables_start_index+len(variable_allowed[i]): self.variables_start_index+self.variables_maxi_num] = 0
                        #print(dimension_mask[i][self.variables_start_index: self.variables_start_index+self.variables_maxi_num])
                    #print(torch.sum(dimension_mask))

                    #dimensionless op
                    dimensionless_allowed = torch.all(
                        required_dimension == self.dimensionless_dimension_index, dim=1
                    )
                    if any(~dimensionless_allowed):
                        mask_idx = torch.meshgrid(torch.arange(bs)[~dimensionless_allowed], self.dimensionless_op_index, indexing="ij") 
                        dimension_mask[mask_idx] = 0
                    #print(torch.sum(dimension_mask))

                    #apply mask
                    masked_scores = scores.clone()
                    masked_scores[dimension_mask == 0] = -torch.inf
                else:
                    masked_scores = scores

                # select next words: sample or greedy
                if sample_temperature is None:
                    next_words = torch.topk(masked_scores, 1)[1].squeeze(1)
                else:
                    next_words = torch.multinomial(
                        F.softmax(masked_scores.float() / sample_temperature, dim=1), num_samples=1
                    ).squeeze(1)
                assert next_words.size() == (bs,)

                # update generations / lengths / finished sentences / current length
                generated1[cur_len] = next_words * unfinished_sents + self.pad_index * (1 - unfinished_sents)
                ...
                if self.use_dimension_mask:
                    ...
                    masked_scores_units[~self.dimension_mask.unsqueeze(0).expand_as(scores_units)] = -torch.inf
                else:
                    masked_scores_units = scores_units

                # select next words: sample or greedy
                if sample_temperature is None:
                    next_words_dim = torch.topk(masked_scores_units, 1)[1].squeeze(-1)
                else:
                    next_words_dim = torch.multinomial(
                        F.softmax(masked_scores_units.reshape(-1, self.n_words).float() / sample_temperature, dim=-1), num_samples=1
                    ).squeeze(-1).reshape(bs, 5)
                assert next_words_dim.size() == (bs,5)
                generated2[cur_len] = next_words_dim * unfinished_sents[:,None] + self.pad_index * (1 - unfinished_sents[:,None])
                ...
                next_dimensions_prob = F.softmax(scores_units.float(), dim=-1)    #(bs, 5, n_words)
                next_dimensions_perplexity = next_dimensions_prob[
                    tuple([
                        torch.arange(bs).unsqueeze(1).repeat((1, 5)).reshape((-1)),
                        torch.arange(5).unsqueeze(0).repeat((bs, 1)).reshape((-1)),
                        next_words_dim.reshape((-1))
                    ])
                ].reshape((bs, 5))                                              #(bs, 5)
                next_dimensions_perplexity = torch.prod(next_dimensions_perplexity, dim=1)
                assert next_words_perplexity.size() == next_dimensions_perplexity.size() == (bs, )

                word_perplexity.add_(
                    torch.log(next_words_perplexity.detach()) * unfinished_sents
                )
                dimension_perplexity.add_(
                    torch.log(next_dimensions_perplexity.detach()) * unfinished_sents
                )
            ...
            rows, cols = torch.nonzero(generated1[1:] == self.eos_index, as_tuple=True)
            word_perplexity = torch.exp(- word_perplexity / rows)
            dimension_perplexity = torch.exp(- dimension_perplexity / rows / 5)

            return generated1[:cur_len], gen_len, generated2[:cur_len], word_perplexity, dimension_perplexity
