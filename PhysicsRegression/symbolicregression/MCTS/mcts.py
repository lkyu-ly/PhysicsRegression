...
class MctsNode():
    def __init__(self, productions, nt, grammar, non_terminal, parent=None):
        ...
        self.children = [0 for _ in range(len(grammar[nt[0]]))] if not self.is_terminal else []
        self.num_children = 0
    ...
    def __repr__(self):
        s = "Node\nexpr:{}\nchildren:{}\nvisits:{}\nrewards:{}".format(self.expr, len(self.children), self.visits, self.rewards)
        return s
...
class MCTS():
    ...
    def extract_subexpr(self, node, update=False):
        # 1. complete subexpr, which is terminal nodes
        # 2. incomplete subexpr, which only consists of symmetric subexpr

        if node is None:
            return

        try:
            # 1. dfs search every node as starting
            dfs_stack = [node]
            sub_expr1 = []
            while dfs_stack:
                current_node = dfs_stack.pop(0)

                # which kind of subexpr don't we want?
                # (1) are simple constant, i.e. no variables
                # (2) ...
            ...
            # 2. dfs search every node to exame symmetry
            dfs_stack = [node]
            sub_expr2 = []
            while dfs_stack:
                current_node = dfs_stack.pop(0)
                ...
                while True:
                    ...
                    if len(s) > 2 or any([len(cc) < 2 for c in current_nodes for cc in c.children]+[len(c) < 2 for c in current_nodes]):
                        break

                    current_nodes = [
                        cc for c in current_nodes
            ...
            for i in range(len(sub_expr1)):
                for k,v in {"add":"+", "sub":"-", "mul":"*", "inv":"1/", "neg":"-"}.items():
    ...
    def extract_original_solution(self, tree, xy):
        if tree is not None:
            e = str(tree)
            for k,v in {"add":"+", "sub":"-", "mul":"*", "inv":"1/", "neg":"-", "pow": "**"}.items():
                e = e.replace(k, v)
    ...
    def const_optimize(self, expr, xy, n_restart=5):
        ...
        #check the dimension of 'skeleton'
        dimension = expr.count("C")
        if dimension == 0: return expr, 1
        ...
        for i in range(n_restart):
            try:
                ...
                res = minimize(loss, x0=x0, args=(x, y), method='bfgs', tol=1e-6)
                if res.success:
                    consts.append(res.x)
    ...
    def UCT(self, parent, child):
        if self.mcts_search_type    == "normal":reward = child.rewards / child.visits 
        elif self.mcts_search_type  == "max":   reward = child.rewards
    ...
    def reward(self, expr, xy, l, f=None):
        ...
        update_expr = expr.replace("pow", "**")
        func = sp.lambdify(x_variables, update_expr, "numpy")
        if f is None:f = self.discount_factor
        try:
            y_pred = func(*x.T) 
    ...
    def reward(self, expr, xy, l, f=None):
        from numpy import exp, log, abs, sin, cos, tan, arcsin, arccos, arctan, sinh, cosh, tanh, arcsinh, arccosh, arctanh, pi
        ...
        try:
            ...
            R = float(f**l / np.sqrt(1 + np.linalg.norm(y.reshape((-1)) - y_pred.reshape(-1), 2) **2 / y.shape[0]))
            R2 = float(f ** l / np.sqrt(1 + np.linalg.norm((y.reshape((-1)) - y_pred.reshape(-1)) / np.maximum(np.abs(y.reshape((-1))), 1e-10), 2) **2 / y.shape[0]))
    ...
    def selection(self, root):
        ...
        while not node.is_expandable and not node.is_terminal and t < self.t_max:
            ...
            UCTs = np.array([self.UCT(node, child) if not child.is_terminal else 0
                            for child in node.children])

            #select best UCT by epsilon greedy
    ...
    def expansion(self, node, nt):
        ...
        p = np.array([1 if child == 0 else 0 for child in node.children], dtype=np.float32)
        p /= np.sum(p)
        ...
        if isinstance(op, str):
            ...
            new_node = MctsNode(node.productions + [[s, op]], deepcopy(nt), self.grammar, self.non_terminal, parent=node)
        elif isinstance(op, list):
            nt = self._next_nt(nt, op)
            new_node = MctsNode(node.productions + op, deepcopy(nt), self.grammar, self.non_terminal, parent=node)
    ...
    def simulation(self, node, xy, nt):
        ...
        for i in range(self.num_simulations):
            simulation_expr, productions = self._simulation(expr, len(node.productions), deepcopy(nt))
            if simulation_expr is None:
                continue
            
            
            optimized_expr, flag = self.const_optimize(simulation_expr, xy, n_restart=1)
            #optimized_expr, flag = simulation_expr, True

            if flag == 0:
                reward = 0
            else:
                reward = self.reward(optimized_expr, xy, l=len(productions)+len(node.productions))
    ...
    def backpropagation(self, node, rewards):
        while node:
            if self.mcts_search_type    == "normal":node.rewards += rewards
            elif self.mcts_search_type  == "max":   node.rewards = max(node.rewards, rewards)  
            node.visits += 1
            node = node.parent

    def search(self, xy, productions=[["O", "A"]], nt=["A"], verbose=False, max_time=-1):
        ...
        self.print_freq = 100
        #pbar = tqdm.tqdm(total=self.search_round)

        for search_round in range(1, self.search_round+1):

            if (search_round) % self.print_freq == 0 and self.print_freq and verbose:
                print(f"\rEpisode {search_round}/{self.search_round}, current best reward {self.best_solution[2]:.4f}.", end="")
                #print(f"\rEpisode {search_round}/{self.search_round}, current best reward {self.best_solution[2]:.4f}, current best expr {self.best_solution[1]}.", end="")
                #sys.stdout.flush()
                #print(f"{search_round}: {self.root.rewards}")
                #pbar.set_description_str(f"current best reward {self.best_solution[2]:.4f}, current best expr {self.best_solution[1]}")
            #pbar.update(1)

            node, flag = self.selection(self.root)
            nt = deepcopy(node.nt)

            #successfully select a node to expand
            if flag:
                cnt[0]+=1
            ...
            #select a node that is too deep 
            #and denied to expand
            else:
                cnt[2]+=1
        ...
        #print()
        #print(f"best expr:{self.best_solution[1]}")
        #print(f"max reward:{self.best_solution[2]}")
