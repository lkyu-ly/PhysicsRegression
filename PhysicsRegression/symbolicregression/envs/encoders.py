...
class Equation(Encoder):
    ...
    def _decode_with_units1(self, lst):
        ...
        if "OOD" in lst[pos]:
            ...
            lst[pos].startswith("CONSTANT") or lst[pos] == "y"
        ):  ##added this manually CAREFUL!!
            ...
            try:
                ...
                return None, 0
            try:
                float(lst[pos])  # if number, return leaf
                node = Node(lst[pos], self.params)
                node.unit = units
                return node, 1 + pos + 5
            except:
    ...
    def decode(self, lst, xy_units=None, decode_physical_units=None, units=None, lable_unit_fn=None):
        trees = []
        ...
        for lst in lists:
            ...
            if decode_physical_units is None:
                ...
                if tree is not None and xy_units is not None:
                    try:    #may output x1+x3 for formula x1+x2
    ...
    def units_encode(self, units, zero_physical_units=False):
        #eg. units = np.array(1,0,0,0,0) / "UNKNOWN_PHYSICAL_UNITS"
