...
class RandomFunctions(Generator):
    def __init__(self, params, special_words):
        ...
        self.variables = ["rand"] + [f"x_{i}" for i in range(self.max_input_dimension)] + ["y"]
        self.symbols = (
            list(self.operators)
            + self.constants
            + self.variables
        ...
        # to preserve just one encoder
        # here we don't concat SPECIAL words to float_words
        ...
        if self.params.dim_length == 2:
            units = ["M"+str(i)+"S"+str(j) for i in range(-20, 21) for j in range(-20, 21)] + \
                ["K"+str(i)+"T"+str(j)+"V"+str(k) for i in range(-9, 10) for j in range(-5, 6) for k in range(-9, 10)]
        elif self.params.dim_length == 5:
            units = ["M"+str(i) for i in range(-30, 31)] + \
        ...
        else:self.differentiation_dic = {}

        #load feynman equations
        ...
        self.physical_units_dic = {
            ...
            for variable, m, s, kg, T, V in 
            zip(df["Variable"], df["m"], df["s"], df["kg"], df["T"], df["V"])
        }
        self.physical_units_dic["pi"] = np.zeros(5)
        self.physical_units_dic["E"] = np.zeros(5)

        #dimensionless variables
        for i in range(10):self.physical_units_dic[f"x_{i}"] = np.zeros(5)
        for i in range(3, 6):self.physical_units_dic[f"theta{i}"] = np.zeros(5)
        ...
        self.unary = [
            "sin", "cos", "tan", "exp", "log", "sqrt", "arcsin", "arccos", "neg", "arctan", 
            "inv", "pow2", "pow3", "pow4", "pow5", "tanh", "sinh", "cosh", "abs"
        ]
    ...
    def add_random_const(self, rng, expr, p_add=0.15, p_mul=0.12):
        ...
        def _add_random_const_strategy2(expr:sp.Expr, args:tuple, type:str, const:float):
            """
            Param:
            ------------
            `expr`: sp.Expr
        ...
        def _recursion(rng, expr:sp.Expr):
            ...
            elif p_a < p_add and not isinstance(expr, sp.core.numbers.Pi) and not isinstance(expr, sp.core.numbers.Number):
    ...
    def infix_to_prefix(self, infix, sp_parse=True, replace_op_dic={}):
        ...
        def _infix_to_prefix(lis):
            #with no brachet
            #return a lis of prefix form

            # case 0: unary
            # always consider simple unary first
            ...
            # case 1: add/sub
            # consider add first, cause sample sub first without brachet may cause problem!
            # consider sub next, and finally neg
            # the add are prefered left/right, but sub can only prefered left
            if "add" in lis or "sub" in lis:
                idx = [i for i in range(len(lis)) if lis[i] == "add" or lis[i] == "sub"]
                idx = idx[-1]
                return lis[idx] + "," + _infix_to_prefix(lis[:idx]) + "," + _infix_to_prefix(lis[idx+1:])
            elif "neg" == lis[0]:
                return lis[0] + "," + _infix_to_prefix(lis[1:])
            
            # case 2: mul/div
            # consider mul first, and inv then
            elif "mul" in lis:
        ...
        for i in range(len(infix)):
            if infix[i] in ["(", ")", "*", "/", "^"] or (infix[i] in ["-", "+"] and infix[i-1] != "e"):
                if temp:
    ...
    def prefix_to_node(self, rng, prefix, variables=None, random_variables_seq=False):
        ...
        prefix = [
            variables_dic.get(p, p) for p in prefix
        ]

        #print(variables)
        #print(dimensions)
        
        #prefix form to node form
    ...
    def post_process(self, node, allow_pow=False):
        ...
        while stack:
            ...
            #some simple power can be reduced
            #eg. x^2...
            if temp.value == "pow":

                try:
                    #must be integer/negative integer
                    f = int(temp.children[1].value)
                    assert 2 <= f <= 5
                    temp.value = "pow" + str(f)
                    temp.children = [temp.children[0]]
                except:
                    assert allow_pow, f"not allowed in pow {temp.children[1]}, not a integer"


            #some negative consts can also be reduced
            elif temp.value == "neg":
                try:
    ...
    def infix_to_node(self, infix, variables=None, allow_pow=False, label_units=True, rng=np.random, random_variables_seq=False, sp_parse=True, replace_op_dic={}):
    ...
    def _dfs_units(self, node, tgt_prefix_units=[], variable_units=[]):
        ...
        if len(children) == 0:
            ...
            if isinstance(op, str) and op[:2] == 'x_':#variable
            ...
            if op in ["add", "sub"]:
                #check const which may have dimension
                try:
                    _ = float(children[0].value)
                    left_const = True
                except:left_const = False
                try:
                    _ = float(children[1].value)
                    right_const = True
                except:right_const = False
                ...
                try:
                    ...
                    is_int = False
                    raise NotImplementedError(f"unsupport operator of pow for labeling dimension!")
    ...
    def generate_multi_dimensional_tree(
        ...
        units = None; consts = None; c_units = None; idx = 0
        ...
        if len(self.to_sample_expr) != 0:
            ...
            self.feynman_exprs_idx = (self.feynman_exprs_idx + 1) % len(self.feynman_exprs)
        ...
        #sample some additive const for sub-expr
        ...
        if self.params.sample_expr_num > 1 and len(self.to_sample_expr) == 0:
            self.to_sample_expr.extend([[expr, idx]] * (self.params.sample_expr_num - 1))
        elif len(self.to_sample_expr) != 0:
            self.to_sample_expr.pop(0)

        #add some frequency sin/cos expr
            ...
            while dfs_stack:
                ...
                if current_node.value in ["sin", "cos"] and len(current_node) == 2:
                    if current_node.children[0].value.startswith("x_"):
                        ...
                        node1 = Node("mul", self.params, children=[
                            node2, current_node.children[0]
                        ])
    ...
    def differentiate(self, tree, original_expr, input_dimension, structure_type):
        assert structure_type in ["add", "mul"], f"wrong structure type: {structure_type}!"
        ...
        for i in range(input_dimension):
            ...
            for j in range(i+1, input_dimension):
                ...
                if isinstance(second_derivative, sp.core.numbers.Number) and abs(second_derivative)<1e-5:
    ...
    def sample_differentiation(self, rng, seperate_idx, sample_prob):
        return [
            idx for idx in seperate_idx if rng.random() < sample_prob
        ]
    ...
    def compute_complexity1(self, tree):
        def _dfs(node):
            ...
            if node.value in self.binary_complex_dic:
                node.complexity = node.children[0].complexity + node.children[1].complexity #+ self.binary_complex_dic[node.value]
            elif node.value in self.unary_complex_dic:
                node.complexity = self.unary_complex_dic[node.value] * node.children[0].complexity
            else:
        ...
        if tree.value in ["add", "sub"]:
            ...
            if not is_float_left and tree.children[1].value in ["mul"]:
                ...
                if is_float_left:
                    ...
                    tree.children[1].unit = tree.children[1].children[0].unit + tree.children[1].children[1].unit
    ...
    def partial_reduce_const(self, tree):
        ...
        if tree.value == "add" and tree.children[1].value == "add":
            ...
            is_f2, f2 = self.is_float(tree.children[1].children[0].value)
            if is_f1 and is_f2:
                tree.children[0].unit = tree.children[0].unit + tree.children[1].children[0].unit
                tree.children[1] = tree.children[1].children[1]
                tree.children[0].value = f1 * f2
    ...
    def partial_reduce_const_inv(self, tree):
        ...
        if tree.value == "mul":
            ...
            if is_f1:
                ...
                if is_f2:
                    tree.children[0].value = f1 / f2
                    tree.children[0].unit = tree.children[0].unit - node.children[0].children[0].unit
    ...
    def _unify_const(self, node, const_node):
        ...
        #stack: (node, inv_flag, additive_node_list)
        #additive_node_list: (node, inv_flag)
        dfs_stack = [(node, 0, [])]
        while dfs_stack:
            ...
            if current_node.value == "mul":
                ...
                if is_f1:
                    ...
                    dfs_stack.append([current_node.children[1], inv_flag, additive_node_list])
                elif is_f2:
                    additive_node_list.append((current_node.children[1], inv_flag))
                    dfs_stack.append([current_node.children[0], inv_flag, additive_node_list])
            else:
                continue

    def unify_const(self, tree):
        # c*(c*v+c)/(c*v+c) -> c*(1*v+c)/(1*v+c)
        start_nodes = []
        const_nodes = []

        #dfs_stack: (node, search_type)
        dfs_stack = [(tree, 0)]
        while dfs_stack:
            ...
            if search_type == 0:
                ...
                if op == "mul":
                    ...
                    if is_f1 or is_f2:
                        dfs_stack.extend(
                            (child, 1) for child in current_node.children
                        )
                    else:
                        dfs_stack.extend(
                            (child, 0) for child in current_node.children
                        )    
                else:
                    dfs_stack.extend(
                        (child, 0) for child in current_node.children
                ...
                #search for another starting node for unify node
                if arity == 1 and op not in [
                    "pow2", "pow3", "pow4", "pow5", "inv", "neg", "sqrt"
                ]:
    ...
    def unify_const2(self, tree):
        ...
        while dfs_stack:
            ...
            if arity == 2:
                is_f1, f1 = self.is_float(current_node.children[0].value, allow_int=False)
                is_f2, f2 = self.is_float(current_node.children[1].value, allow_int=False)
    ...
    def unify_const3(self, tree):
        ...
        while dfs_stack:
            ...
            if op == "inv":
                dfs_stack.extend([(child, not is_inv) for child in current_node.children])
            else:
                dfs_stack.extend([(child, is_inv) for child in current_node.children])
    ...
    def relabel_variables(self, tree, variables):
        ...
        if input_dimension == 0:
            return 0, None
        transition_dic = {
            f"x_{variables.index(v)}": f"x_{j}"
            for j, v in enumerate(active_variables)
        }
    ...
    def function_to_skeleton(
        self, tree, skeletonize_integers=False, constants_with_idx=False, ignore=[], ignore_pow=False
    ):
        ...
        while dfs_stack:
            ...
            try:
                ...
                is_float = True
                if f == int(f):
                    #just refine float or refine even the integer?
                    is_float = True
                if f in ignore:
                    is_float = False
            except ValueError:
            ...
            if is_float or (pre is self.constants and skeletonize_integers):
                ...
                while (prefix and prefix[-1] in self.unaries) or (
                    len(prefix) >= 2 and (
                        self._is_float(prefix[-1]) or prefix[-1].startswith("CONSTANT")
                    ) and prefix[-2] in self.binary
                ):
                    
                    if prefix[-1] in self.unaries and not (prefix[-1] == "inv" and value == 0):
                        if prefix[-1] == "neg":
                            value = - value
                        elif prefix[-1] == "inv":
                            value = 1 / value
        ...
        if units[0] is None:
            ...
            new_tree = self.equation_encoder.decode(prefix, decode_physical_units="skeleton", units=units)
        #if self.params.units_criterion_constrain == 1 or True:
        #    self.label_units_with_consts(new_tree, units)
        return new_tree, constants

    def wrap_equation_floats(self, tree, constants):
        transition_dic = {
            f"CONSTANT_{j}": constants[j]
    ...
    def _generate_datapoints(
        ...
        rng,
        input_dimension,
        input_distribution_type,    #[positive/all, multi/single, normal/uniform]
        n_centroids,
        max_trials,
        rotate=True,
        offset=None,
        ...
        while remaining_points > 0 and trials < max_trials:
            ...
            # decide each centroids
            if trials % 100 == 0:
                if input_distribution_type[1] == "single":
                    # single centroid
                    # sample datapoint from almost all the space
                    n_centroids = 1 
                    ...
                    # multi centroid
                    # for each centroids, sample datapoint from a narrow space
                ...
                if rotate:
                    ...
                    rotations = [np.identity(input_dimension) for _ in range(n_centroids)]

                weights = rng.uniform(0, 1, size=(n_centroids,))
                weights /= np.sum(weights)
                n_points_comp = rng.multinomial(n_points, weights)

            # sample datapoint from each centroids
            ...
            if input_distribution_type[0] == "all":
                ...
                #positive_idx = np.all(input>0, axis=1)
