...
from .generators import all_operators, math_constants, Node
...
class Simplifier:
    ...
    def tree_to_sympy_expr(self, tree, safely_wrap=False):
        prefix = tree.prefix().split(",")

        if safely_wrap:
            prefix = [[p] if p not in ["sqrt", "log"] else [p, "abs"] for p in prefix]
            prefix = [pp for p in prefix for pp in p]
    ...
    def safely_tree_to_torch_module(self, tree, dtype=torch.float32):
