...
from ..utils import MyTimeoutError, bool_flag, timeout
from .utils import *
...
class FunctionEnvironment(object):
    ...
    def mask_from_seperator(self, x, sep):
        ...
        alen = (
            paddle.arange(x.shape[0], dtype=paddle.long, device=x.device)
            .unsqueeze(-1)
            .repeat(1, x.shape[1])
        )
        sep_id_occurence = paddle.tensor(
            [
                (
                    paddle.where(x[:, i] == sep_id)[0][0].item()
                    if len(paddle.where(x[:, i] == sep_id)[0]) > 0
                    else -1
                )
    ...
    def word_to_idx(self, words, float_input=True, unit_input=False):
        if unit_input:
            return [
                paddle.LongTensor(
                    [[self.float_word2id[dim] for dim in point] for point in seq]
                )
                for seq in words
            ]
        if float_input:
            return [
                [
                    paddle.LongTensor([self.float_word2id[dim] for dim in point])
                    for point in seq
                ]
                for seq in words
            ]
            ...
            return [
                paddle.LongTensor([self.equation_word2id[w] for w in eq])
                for eq in words
            ]
    ...
    def _gen_expr(
        ...
        (
            ...
            rng=self.rng, expr=params.expr_data, datatype=datatype
        )
        ...
        if n_input_points is None:
            n_input_points = (
                params.max_len
                if not train
                else self.rng.randint(params.max_len // 2, params.max_len + 1)
            )
        if train:
        ...
        if "units" in use_hints:
            if params.sample_dimension_hints == None:
                ...
                if self.rng.random() < zero_units_prob:
                    zero_units = True
                    units = [
                        np.array([0, 0, 0, 0, 0]) for k in range(len(real_variables))
                    ]
                    y_unit = np.array([0, 0, 0, 0, 0])
                else:
                    zero_units = False
                    units = [
                        self.generator.physical_units_dic[k] for k in real_variables
                    ]
                    y_unit = tree.unit
                ...
                assert (
                    len(units) == len(real_variables) + 1
                ), "wrong physical units num for variables"
        else:
            zero_units = False
            units = None
        if "complexity" in use_hints:
            if params.sample_complexity_hints == None:
                ...
                sigma_complexity = self.generator.compute_complexity2(
                    tree
                ) + self.rng.normal(0, sigma_complexity_sample)
                ...
                complexity = [
                    type_complexity
                    if self.rng.random() > unknown_complexity_prob
                    else 0
                ]
            else:
                assert params.sample_complexity_hints in [
                    "simple",
                    "middle",
                    "hard",
                    "unknown",
                ...
                complexity = [
                    params.sample_complexity_hints
                    if params.sample_complexity_hints != "unknown"
                    else 0
                ]
        else:
            complexity = None
        if "unarys" in use_hints:
            if params.sample_unary_hints == None:
                ...
                unary_ops = [
                    u
                    for u in set(unary_ops[0])
                    if self.rng.random() > unary_abandon_prob
                    and u not in ["neg", "inv", "sqrt", "pow2", "pow3", "pow4", "pow5"]
                ]
                unary_ops = [u for u in self.generator.unary if u in unary_ops]
            else:
                unary_ops = params.sample_unary_hints
                if unary_ops == "":
                    unary_ops = []
                else:
                    unary_ops = unary_ops.split(",")
                assert (
                    isinstance(unary_ops, list)
                    and all([(u in self.generator.unary) for u in unary_ops])
                    and all([(u not in ["neg", "inv"]) for u in unary_ops])
                )
        else:
            unary_ops = None
        if "add_structure" in use_hints or "mul_structure" in use_hints:
            if params.sample_structure_hints == None:
                seperate_idx_add = self.generator.differentiate(
                    tree, original_expr, input_dimension, "add"
                )
                seperate_idx_mul = self.generator.differentiate(
                    tree, original_expr, input_dimension, "mul"
                )
                seperate_idx_add = self.generator.sample_differentiation(
                    self.rng, seperate_idx_add, sample_prob=0.95
                )
                add_structure = self.generator.find_expr_structure(
                    self.rng, seperate_idx_add, input_dimension
                )
                seperate_idx_mul = self.generator.sample_differentiation(
                    self.rng, seperate_idx_mul, sample_prob=0.6
                )
                mul_structure = self.generator.find_expr_structure(
                    self.rng, seperate_idx_mul, input_dimension
                )
            else:
        ...
        if "consts" in use_hints:
            if params.sample_const_hints == None:
                if variable_consts is not None and (
                    zero_units or self.rng.random() < 0.06
                ):
                    const = []
                    ...
                    const = [
                        ...
                        if self.rng.random() < const_sample_rate
                    ]
                    const = [
                        (
                            [float(_value), _units]
    ...
    def register_args(parser):
        ...
        parser.add_argument(
            "--max_unary_ops", type=int, default=4, help="Max number of unary operators"
        )
        parser.add_argument(
            "--min_op_prob",
            type=float,
...
class EnvDataset(paddle.io.Dataset):
    def __init__(
        self,
        env,
        task,
        ...
        if self.path is None:
            ...
            while n > 0:
                if len(self.samples) == 0:
                    ...
                    if self.count_queue_num == 1080:
                        self.count_queue_num = 1000
                    with open(
                        f"{self.path}queue_{self.count_queue_num}.pkl", "rb"
                    ) as fi:
                        self.samples = pickle.load(fi)
                    for i in range(len(self.samples)):
                        idx = np.random.randint(
                            0,
                            self.samples[i]["x_to_fit"].shape[0]
                            - self.params.max_len
                            + 1,
                        )
                        self.samples[i]["x_to_fit"] = self.samples[i]["x_to_fit"][
                            idx : idx + self.params.max_len
                        ]
                        self.samples[i]["y_to_fit"] = self.samples[i]["y_to_fit"][
                            idx : idx + self.params.max_len
                        ]
                        self.samples[i]["infos"][
                            "n_input_points"
                        ] = self.env.params.max_len
                        self.samples[i]["infos"][
        ...
        if max_size < 0:
            ...
            if size > max_size:
                if after > before + 1:
                    ...
                    logger.warning(
                        f"Exceeding tokens_per_batch: {size} ({after - before} sequences)"
                    )
        return after
    ...
    def collate_fn(self, elements):
        ...
        info_tensor = {
            info_type: paddle.LongTensor(samples["infos"][info_type])
    ...
    def init_rng(self):
        ...
        if self.train:
            ...
            logger.info(
                f"Initialized random generator for worker {worker_id}, with seed {seed} (base seed={self.env_base_seed})."
            )
        else:
            worker_id = self.get_worker_id()
            self.env.worker_id = worker_id
